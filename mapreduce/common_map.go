...
func doMap(
	...
	// call user map func for generate kvs
	kvs := mapF(inFile, string(contents))

	// create nReduce intermediate file
	...
	for i := range interFile {
		...
		interFile[i] = file
		defer interFile[i].Close()
	}

	// According to the hash of the key stored in the corresponding files
	for _, kv := range kvs {
